...
nsoCurrentSDM <- dismo::maxent(x = presenceAbsenceEnvDf, ## env conditions
...
nsoPredictPlot <- raster::predict(nsoCurrentSDM, geographicArea) 


# convert to a spatial pixels data frame
raster.spdf <- as(nsoPredictPlot, "SpatialPixelsDataFrame")

# convert to data frame
nsoPredictDf <- as.data.frame(raster.spdf)
...
ggplot() +
  geom_polygon(data = wrld, mapping = aes(x = long, y = lat, group = group),
               fill = "grey75") +
  geom_raster(data = nsoPredictDf, aes(x = x, y = y, fill = layer)) + 
...
# save it!
ggsave("output/nsoCurrentSdm.jpg",  width = 8, height = 6)
